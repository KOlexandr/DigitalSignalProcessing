...
object FourierTransform {
  ...
  def fftd(x: Array[Double]): Array[Double] = fft(x.map(C(_, 0))).map(_.abs)
  /**============================================ Fast Fourier Transform ===========================================**/
  ...
  def dft(x: Array[Double]): Array[Complex] =
    (for {i <- x.indices} yield dftOneElement(x, x.length, i)).toArray.map(_.conjugate)
  ...
  private def bitReverseCopy(x: Array[Complex]): Array[Complex] = {
    ...
    val size: Int = Integer.toBinaryString(x.length).length
    for {k <- x.indices} res(reverseBits(k, size)) = x(k)
    res
  }

  private def reverseBits(x: Int, size: Int): Int = {
    val string: String = Integer.toBinaryString(x)
    Integer.parseInt((Array.fill(size - string.length - 1)(0).mkString("") + string).reverse, 2)
  }
